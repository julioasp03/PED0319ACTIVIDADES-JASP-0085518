...
int main(void){
    ...
    do{
        ...
        cout << "5) Ordenar.\n";
        cout << "6) Mostrar articulos que tienen fecha de vencimiento.\n";
        cout << "7) Mostrar articulos que pesan mas de 300 gramos.\n";
        cout << "8) Mostrar articulos que  que valen mas de 50 dolares.\n";
        cout << "9) Salir.\n";
        cout << "Opcion: ";
...
void agregar(){
    ...
    cout << "Tiempo en bodega(anios): "; cin>>unArticulo.tiempo; cin.ignore();
...
void mostrar(){
    for (int i = 0; i < bodega.size(); i++) {
        ...
        cout << " gramos , vale $" << bodega[i].precio <<" y tiene "<<bodega[i].tiempo<<" anios en bodega\n";
    }
}
void ordenar(){
    int opcion = 0;
...
void vencimiento(){
    for (int i = 0; i < bodega.size(); i++)
    {
        if(bodega[i].tiempo>=5){
            cout<<"El articulo "<<bodega[i].nombre<<" Tiene "<<bodega[i].tiempo<<"en bodega por lo cual ya expiro"<<endl;
        }
        else
        {
            cout<<"No hay un artiulo que haya expirado\n";
        }
...
void peso(){
    for (int i = 0; i < bodega.size(); i++)
    {
        if(bodega[i].peso==300){
            cout<<"El articulo "<<bodega[i].nombre<<" Pesa: "<<bodega[i].peso<<endl;
        }
        else
        {
            cout<<"No hay un artiulo que pese 300 gr\n";
        }
...
void precio(){
    for (int i = 0; i < bodega.size(); i++)
    {
        if(bodega[i].precio>=50){
            cout<<"El articulo "<<bodega[i].nombre<<" Cuesta: "<<bodega[i].precio<<endl;
        }
        else
        {
            cout<<"No hay un artiulo que cueste mas 50$\n";
        }
